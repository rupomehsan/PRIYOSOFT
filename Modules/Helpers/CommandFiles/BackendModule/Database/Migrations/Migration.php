...
if (!function_exists('Migration')) {
    function Migration($moduleName, $fields)
    {
        ...
        if (count($fields)) {
            foreach ($fields as $fieldName) {
                ...
                if (count($fieldName) > 1) {
                    ...
                    // Handle decimal-N.M (e.g. decimal-15.2 → decimal(15,2))
                    if (strpos($type, 'decimal-') === 0) {
                        $ps = explode('.', explode('-', $type, 2)[1]);
                        $decimalPrecision = (int) ($ps[0] ?? 15);
                        $decimalScale     = (int) ($ps[1] ?? 2);
                        $type = 'decimal';
                    }
                    ...
                    } else if ($type == 'decimal' && $decimalPrecision !== null) {
                        $content .= "            \$table->decimal('{$fieldName[0]}', {$decimalPrecision}, {$decimalScale})->nullable();\n";
                    } else {
        ...
        // Only add built-in columns if not already declared as user fields
        $fieldNames = array_column($fields, 0);
        if (!in_array('creator', $fieldNames)) {
            $content .= "                    \$table->bigInteger('creator')->unsigned()->nullable();\n";
        }
        if (!in_array('slug', $fieldNames)) {
            $content .= "                    \$table->string('slug', 50)->nullable();\n";
        }
        if (!in_array('status', $fieldNames)) {
            $content .= "                    \$table->enum('status', ['active', 'inactive'])->default('active');\n";
        }
        $content .= <<<EOD
                    \$table->timestamps();
                    \$table->softDeletes();
