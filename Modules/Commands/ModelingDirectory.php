...
class ModelingDirectory extends Command
{
    ...
    */
    ...
    protected function initializeProperties()
    {
        ...
        // Reset all accumulated state — required when command is reused as a singleton
        $this->fields           = [];
        $this->fileFields       = [];
        $this->jsonFields       = [];
    ...
    protected function parseFields()
    {
        ...
        foreach (explode(',', $arg) as $item) {
            ...
            // Strip {ModulePath} FK annotation from field name, record relationship
            if (preg_match('/^([^{]+)\{([^}]+)\}$/', $rawName, $m)) {
                $cleanName    = $m[1];
                $braceContent = str_replace('/', '\\', $m[2]);
                $this->fieldsWithBraces[] = [
                    'field'         => $cleanName,
                    'brace_content' => $braceContent,
                ];
            } else {
                $cleanName = $rawName;
            }
